...
## Atualização — sessão de auditoria (`chore/auditoria-tecnica-pt-br`)

Foram criadas três novas instructions de conteúdo (data-driven), aplicadas
automaticamente quando o agente edita arquivos correspondentes:

| Instruction | applyTo | Função |
|---|---|---|
| `.github/instructions/conteudo-evento.instructions.md` | `data/eventos.json`, `eventos/**/*.html` | Regras editoriais para eventos (campos obrigatórios, fluxo validate→build) |
| `.github/instructions/conteudo-artigo.instructions.md` | `data/artigos.json`, `artigos/**/*.html` | Regras para artigos do blog |
| `.github/instructions/conteudo-agenda.instructions.md` | `data/agenda-pastoral.json`, `data/horarios-missa.json` | Regras para agenda pastoral e horários de missa |

### Novos comandos disponíveis (npm scripts)

| Comando | Propósito |
|---|---|
| `npm run validate-data` | Valida `data/*.json` contra `schemas/*.schema.json` (draft-07) |
| `npm run build:content` | Gera HTMLs em `eventos/`, `artigos/`, `homilias/` a partir dos JSONs |
| `npm run all` | Pipeline completo: validate-data → build:content → build (partials) → seo |

### Convenções para os agentes

- **Nunca** editar diretamente HTMLs gerados em `eventos/`, `artigos/`, `homilias/` — eles têm banner `<!-- GENERATED BY scripts/build-content.js -->` e são sobrescritos.
- Para criar/alterar evento, artigo ou homilia: editar `data/<colecao>.json`, rodar `npm run validate-data`, depois `npm run build:content`.
- Templates ficam em `templates/` (somente leitura para o agente padrão; agentes especializados de UI podem alterá-los, mas devem rodar `build:content` em seguida).
- Componentes dinâmicos (`data-component="horarios-missa"`, `data-component="agenda-pastoral"`) leem JSON em runtime — não exigem rebuild ao editar dados.

### Spike CMS (`cms/`)

Esqueleto Laravel + Filament documentado em [`cms/README.md`](cms/README.md).
Agentes não devem rodar `composer install` nessa pasta sem autorização
(introduz `vendor/` que não pertence ao site público).
