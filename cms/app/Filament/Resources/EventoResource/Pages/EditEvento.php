        ...
        return [
            Actions\Action::make('previsualizar')
                ...
                ->action(function () {
                    ...
                    // Converte objetos Carbon para string ISO
                    foreach ($item as $k => $v) {
                        if ($v instanceof \DateTime) {
                            $item[$k] = $v->format('Y-m-d');
                        }
                    }

                    file_put_contents('/tmp/paroquia_preview_in.json', json_encode($item));
                    exec('node /root/dev/paroquiansrjericopb/scripts/build-content.js --preview-stdin --type eventos 2>&1 < /tmp/paroquia_preview_in.json', $out, $exit);
                    ...
                    $this->js("
                        ...
                        o.innerHTML = `
                            <div style='width:93%;height:93%;background:#fff;border-radius:10px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 64px rgba(0,0,0,.6)'>
                                <div style='display:flex;align-items:center;justify-content:space-between;padding:10px 18px;background:#1e3a5f;color:#fff;flex-shrink:0'>
